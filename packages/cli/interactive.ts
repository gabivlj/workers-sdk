...
import { blue, dim, gray, brandColor, bold, white } from "./colors";
import SelectRefreshablePrompt from "./select-list";
import { cancel, space, status, newline, shapes } from "./";
import type { Option, OptionWithDetails } from "./select-list";
import type { Prompt } from "@clack/core";

export type Arg = boolean | string | undefined | string[] | number;
		...
		return [
			...
			`${
				cursor > 0 && options.length > maxItemsPerPage
					? `${space(2)}${dim("...")}\n`
					: ""
			}${options.map(renderOption).filter(renderOptionCondition).join(`\n`)}${
